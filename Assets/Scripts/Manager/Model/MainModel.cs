	...
	*/
	public class MainModel : MonoBehaviour
	{
		...
		*/
		...
		{
			//Evento de Partido: Fin de partido
		...
		{	/* Versíón Football Star*///    get { return LoadSaveModeJSON? "PlayerDataJSONV1" : "PlayerDataBinV1"; }
			/* Versíón Jugador5Estrellasr*/ get { return LoadSaveModeJSON? "J5EPlayerDataJSONV1" : "J5EPlayerDataBinV1"; }
		}
		...
		{			
			...
			if (header != null) {
				header.GetComponent<Header> ().GoToMainScreen ();
			} else {
				Debug.LogError("No se ha encontrado la GUI del Header");
			}
		}
		...
		*/
		...
		{
			int idx = (int)GlobalDifficultyIndex (matchDef);
			//Debug.LogError (">>> Global difficulty name: " + idx);

			return DIFF_NAMES[idx];
		}
		...
		IEnumerator PlayMatchCoroutine(Match match)
		{
			...
			/*
			 // GENETSIS ES E ANTIGUO TRACKER DE MAHOU... LO DESACTIVAMOS POR DEPRECATED
			if ( Genetsis != null )
			...
			*/
            ...
            mMatchBridge.CurrentMatchDefinition.Nick = Nick;
		...
		*/
		void OnApplicationPause(bool suspended)
		{
			if (suspended) 
			{
				...
				// Evento de Menu: Aplicacion pausada
		...
		public Match LastMatchPlayed {
			get { return mLastMatchPlayed;}
			private set { mLastMatchPlayed = value; }
		}
