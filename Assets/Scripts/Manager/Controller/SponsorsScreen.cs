...
namespace FootballStar.Manager
{
	public class SponsorsScreen : MonoBehaviour
	{
        ...
        public Shader UnlitTextureShader;
        public UITexture mBackgroundTexture;
		...
		void RefreshContent()
		{
			mBackgroundTexture.material = new Material(UnlitTextureShader);
			mBackgroundTexture.mainTexture = mCurrentTierBackgrounds[mCurrentButtonIdx];
            mBackgroundTexture.width = 1280;
            mBackgroundTexture.height = 720;
		...
		void RefreshBackgroundsForTier(int tierIdx)
		{
			...
			{
				var textureName = String.Format("SponsorTier3Background{0:00}", c + 1);
