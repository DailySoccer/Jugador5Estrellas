...
namespace FootballStar.Manager
{
	public class PlayScreenRightBox : MonoBehaviour
	{
		...
		void ForceUpdate()
		{
            ...
            mMainModel.CurrentTier.Definition.OwnTeamID = mMainModel.SelectedTeamId;
			...
			else
			{
				...
				int numBalls = mMatchToDisplay.MatchResult.Stars;
				...
				SponsorsPerTier sponsors = mMainModel.Player.CurrentTier.Sponsors;
				mHelpText.text = (numBalls < mBalls.Count()) ? 
						(sponsors.PurchasedSponsorIDs.Count > 0 ? 
							"Vuelve a jugar para aumentar tus [FFCB05]fans\n[FFFFFF]y conseguir más [FFCB05]dinero." 
						: 
							"Vuelve a jugar para conseguir más [FFCB05]Fans.") 
					: 
						sponsors.PurchasedSponsorIDs.Count > 0 ? 
							"Juega de nuevo y\n consigue más [FFCB05]dinero." 
						: 
							"Vuelve cuando tengas un [FFCB05]patrocinador [FFFFFF] y así podrás conseguir mas [FFCB05]dinero[FFFFFF]";
			}
		}
