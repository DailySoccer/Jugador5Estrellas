...
namespace FootballStar.Manager
{
	public class Header : ScreenStack
	{
		...
		void HandleOnLastMatchResult(object sender, LastMatchResultEventArgs e)
		{
			...
			// TODO: Si el ̼ltimo partido jugado era de dificultad media o extrema y he perdido -> Muesto la ventana de necesitas mejorar
			if (mMainModel.LastMatchPlayed != null) {
				if (mMainModel.LastMatchPlayed.MatchResult == null &&   // Si no hemos ganado el partido (ahora o nunca) 
				    mMainModel.LastMatchPlayed.NumTimesPlayed > 0 &&    // Si lo hemos jugado alguna vez
				    isMatchDifficulGraterThan(mMainModel.LastMatchPlayed.Definition, FootballStar.Common.MatchBridge.Difficulty.MEDIUM)) { // Si la dificultad fue ALTA o EXTREMA
					StartCoroutine(ShowNeedToImproveModal());
				}
			}
		}

		IEnumerator ShowNeedToImproveModal() {
			while (!CameraFade.Finished)
				yield return null;

			mNeedToImproveMessageOverlap = NGUITools.AddChild(this.gameObject, YouNeedToImprovePrefab);
			mNeedToImproveMessageOverlap.GetComponentInChildren<UIButtonMessage>().target = this.gameObject;
		...
		*/
		...
		private void ShowEnergyChangeEffect(int amount)  
		{
			if(amount != 0)
			{
				...
				//TODO: No mostrar si hay ventanas por encima.
				if (!IsOverlapedScreen())
		...
		*/
		...
		void PutOutOfEnergyScreen()
		{
			...
			mNeedEnergyMessageOverlap = NGUITools.AddChild(this.gameObject, YouNeedEnergyScreen);
			mNeedEnergyMessageOverlap.GetComponentInChildren<UIButtonMessage>().target = this.gameObject;
			mNeedEnergyMessageOverlap.GetComponentInChildren<YouNeedEnergy>().LastPlay = mMainModel.Player.LastEnergyUse;
			mNeedEnergyMessageOverlap.GetComponentInChildren<YouNeedEnergy>().OnEnergyCountdownEnds += HandleOnEnergyCountdownEnds;
			mNeedEnergyMessageOverlap.transform.localPosition = new Vector3(0, 0, -1000);
		}
		...
		*/
        public void SetupHudForSelectedTeam() {
            ...
            PlayerName.text = mMainModel.Nick;
