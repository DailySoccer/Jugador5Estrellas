...
namespace FootballStar.Manager
{
	...
	{	
		...
		public GameObject CompetitionLocked_Modal;
		...
		{			
			if (mMainModel.Player.TutorialStage == TutorialStage.DONE) {
				...
				OnFriendlyClick ();
			} else if (mMainModel.Player.TutorialStage == TutorialStage.CONTROLS_EXPLANATION) {
				mCheckboxFriendly.value = true;
				OnFriendlyClick ();
		...
		void OnContinueMatchExplanationClick() {
			//Evento tutorial: Partido amistodo
		...
		void OnContinueAllCompetitionsOpenedClick() {
			//Evento tutorial: Todo desbloqueado
		...
		void OnContinueCompetitionsLockedClick() {
			Destroy (mMessageOverlap);
			mMessageOverlap = null;
		}

		void ShowCompetitionLockedModal() {
			mMessageOverlap = NGUITools.AddChild (this.gameObject, CompetitionLocked_Modal);
			mMessageOverlap.GetComponentInChildren<UIButtonMessage> ().target = this.gameObject;
		}
