...
namespace FootballStar.Manager
{
	public class ImprovementScreen : MonoBehaviour
	{
		...
		{				
			...
			mBuyButton = transform.FindChild("Left Panel").FindChild("Lower Box").FindChild("Buy Button").gameObject;
			mUIBuyButton = mBuyButton.GetComponent<UIButton>();

			mSoldOutContainer  = transform.FindChild("Left Panel").FindChild("Lower Box").FindChild("SoldOut Container").gameObject;
			mForbiddenContainer = transform.FindChild("Left Panel").FindChild("Lower Box").FindChild("Forbidden Container").gameObject;
		}
		...
		void OnBuyButtonClick()
		{
			...
			parameters.Add(CurrentItem.Name, "comprado");
			AppsFlyer.trackRichEvent(CurrentItem.Name, parameters);
		...
		void ForceUpdate()
		{
			...
			if (mMainModel.Player.Improvements.IsItemAlreadyPurchased(theItem))
			{
				...
				SetBackground(false, theItem, true);
			}
			else
			{
				...
				SetBackground(locked, theItem, false);
			}
		}


		private void SetBackground(bool isLocked, ImprovementItem theItem, bool owned)
		{
			if (theItem.Background != null)
			{
				...
				mBackgroundTexture.mainTexture = owned ? theItem.BackgroundWhenOwned : theItem.Background;
			}
			else
			{
