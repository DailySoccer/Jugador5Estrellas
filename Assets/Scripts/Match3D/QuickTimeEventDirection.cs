...
{	
	public class QuickTimeEventDirection : MonoBehaviour {
		...
		public void Deactivate() {
            mMatchManager.GetComponent<TimeScale>().factor = 1f;
			mState = States.OFF;
		}
...
//                CircleRenderer.TimerSync = (x) => mCurrentTimer;
                ...
                mDirectionGesture.AngleMax = DifficultyParams[Difficulty].angle;
		...
		public void NotifyAction(bool success) {
			StopAllCoroutines();
            float val = 1.0f - ((mCurrentTimerEnd - mCurrentTimer) / mCurrentTimerEnd);
