...
namespace FootballStar.Match3D
{
   ...
   public class EventInteractiveActionArgs : EventArgs
   {
      ...
      static public InteractionResponseKind EvaluateResult(float result)
      {
         ...
         if (result > 0.5f)
            kind = InteractionResponseKind.GOOD;
         else
            kind = InteractionResponseKind.BAD;
         return kind;
      }
   }
   ...
   public class InteractiveActions : MonoBehaviour
   {
      ...
      void Start()
      {
         ...
         else
         {
				if (mQTEDirection != null)
				{
					Debug.Log("<color=red>No " + mQTEDirection.GetType() + " object detected.</color>");
				}
         }
      ...
      public void MsgBegin(InteractionResponseKind kind)
      {
         if (OnEvent != null)
         {
            EventInteractiveActionArgs args = new EventInteractiveActionArgs(InteractionStates.BEGIN);
            args.ResultKind = kind;
            OnEvent(this, args);
         }
      }
