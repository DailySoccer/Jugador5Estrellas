...
namespace FootballStar.Match3D {
	...
	public class Customization : MonoBehaviour {
        ...
        public void Setup() {
         ...
         InstantiateFutbolistas(mPerformanceTuner.OwnQualityLevel >= 3);
		...
		void Initialize() {
			mAttackers = new List<GameObject>(11);
			mDefenders = new List<GameObject>(11);
		}

      public void InstantiateFutbolistas(bool highQuality)
      {
         if (_playersHolder == null)
         {
            ...
            GameObject soccer;
            _playersHolder = new GameObject("FootballPlayers");
            _playersHolder.transform.SetParent(transform);
            for (int i = 0; i < 11; ++i)
            {
               soccer = Instantiate(highQuality ? SoccerPrototype : SoccerPrototypeLow) as GameObject;
               soccer.tag = "Local";
               soccer.name = "Local" + (i + 1);
               soccer.transform.SetParent(_playersHolder.transform);
               //soccer.transform.position = Vector3.forward * (i + 1);
               if (i == 0)
               {
               ...
               else
               {
                  ...
                  ChangeMaterial(soccer, null, originalLocalPixels, i + 1, true);
                  ...
                  {
                     CreatePelo(soccer.transform, i + 1);
                  }
               }
               mAttackers.Add(soccer);
               soccer = Instantiate(highQuality ? SoccerPrototype : SoccerPrototypeLow) as GameObject;
               soccer.tag = "Defender";
               soccer.name = "Defender" + (i + 1);
               soccer.transform.SetParent(_playersHolder.transform);
               //soccer.transform.position = Vector3.forward * -(i + 1);
               ...
               if (i == 0)
                  ...
                  ChangeMaterial(soccer, MaterialsDefender, originalDefenderPixels, i + 1, false);

               CreatePelo(soccer.transform, i + 1);
               mDefenders.Add(soccer);
            }
         }
      }
      ...
      void ChangeMaterial( GameObject soccer, SoccerCustomMaterials soccerCustomMaterials, Color32[] equipacionOriginal, int dorsal, bool cacheable ) {
        ...
        public List<GameObject> Attackers { get { return mAttackers; } }
        List<GameObject> mAttackers;
        public List<GameObject> Defenders { get { return mDefenders; } }
        List<GameObject> mDefenders;

        MatchBridge mMatchBridge;
		EquipacionesCache mEquipacionesCache;
		PerformanceTuner mPerformanceTuner;

      private GameObject _playersHolder;
