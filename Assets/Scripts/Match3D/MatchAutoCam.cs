...
//			if (mMode != eMode.MATCH_FAILED) StartCoroutine(LookUpToScoreboardCoroutine());
        ...
        void UpdatePassMode(bool forced = false) {
            ...
            if (daPoint.BallMotor.NewPropietary != null)
                finalLookAtPos = Vector3.Lerp(daPoint.BallMotor.NewPropietary.transform.position, new Vector3(55, 1, 0), 0.35f);
            else
                finalLookAtPos = Vector3.Lerp(mLastPlayerWithBall.transform.position, new Vector3(55, 1, 0), 0.3f);
            ...
            if (daPoint.BallMotor.NewPropietary != null)
                mLastPlayerWithBall = daPoint.BallMotor.NewPropietary;
        }

        static Transform oldOwner = null;
        void UpdateDribbleMode(bool forced = false) {
            ...
            if (daPoint.Source != null)
                ...
                p = Vector3.Lerp( mLastPlayerWithBall.transform.position, daPoint.BallMotor.transform.position,0.7f);
            
            Vector3 finalLookAtPos = Vector3.Lerp(p, new Vector3(55, 1, 0), 0.45f);
            Vector3 dif = finalLookAtPos - p;
            dif.y = 0;
            Vector3 dir = dif.normalized;
            Vector3 end = p - dir * 15.0f;
        ...
        void UpdateShotMode(bool forced = false) {
            var daPoint = mMatchManager.PointOfInterest;
            if (daPoint.BallMotor.NewPropietary != null)
                mLastPlayerWithBall = daPoint.BallMotor.NewPropietary;
            ...
            // 3rd person mode.
            Vector3 dif = targetPos - mLastPlayerWithBall.transform.position;
            dif.y = 0;
            Vector3 dir = dif.normalized;
            Vector3 end;
            Vector3 ball = daPoint.BallMotor.transform.position;
            if (daPoint.BallMotor.NewPropietary != null)
            {// Antes del tiro
                end = mLastPlayerWithBall.transform.position - dir * 10.0f;
            }
            else // Despues del tiro
            {
                end = mLastPlayerWithBall.transform.position - dir * 3.0f;
            }
            targetPos.z = targetPos.z * 0.8f + ball.z * 0.2f;
            end.y = 2.5f;
