...
namespace FootballStar.Match3D {
	...
	public class MatchEndDialog : MonoBehaviour 
	{
		...
		IEnumerator DoTheSequence() 
		{
			...
			StartCoroutine(AnimateCoordX(ContinueButtonHandle, ContinueButtonHandle.localPosition.x - 220.0f, 0.150f));


			// No activamos FB (Activar si se requiere y chequear qeu la App en developer.facebook.com sigue funcionando)
			...
			int nuBalls = mMatchManager.MatchResult.Stars;
			int numPrevBalls = 0;
			int sumFans = 0;
			...
			if (mMatchBridge != null && mMatchBridge.CurrentMatchPrevResult != null)
				numPrevBalls = mMatchBridge.CurrentMatchPrevResult.Stars;
		...
		private IEnumerator OnContinueCoroutine()
		{
			...
			parameters.Add(AFInAppEvents.END_PLAY, "terminar");
			AppsFlyer.trackRichEvent(AFInAppEvents.END_PLAY, parameters);
