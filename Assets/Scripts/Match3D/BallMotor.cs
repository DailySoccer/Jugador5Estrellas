...
namespace FootballStar.Match3D {
	...
	public class BallMotor : MonoBehaviour {
		...
		void Update () {
			...
			if ( transform.parent == null ) {
				...
				if ( MovingToPosition && TimeToTarget <= 0f ) {
                    ...
                    if (Estado == EEstado.Chut) {
                        if (Gol)
                            GameObject.FindGameObjectWithTag("MatchManager").GetComponent<MatchManager>().Gol();
                        else
                            GameObject.FindGameObjectWithTag("MatchManager").GetComponent<MatchManager>().Fail();
                    }
		...
		public void UnAttach (BallMotor.EEstado estado = BallMotor.EEstado.Ninguno) {
            NewPropietary = null;
            Estado = estado;
