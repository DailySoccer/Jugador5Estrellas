...
namespace FootballStar.Audio {
	...
	public class AudioInGameController : MonoBehaviour 
	{
		...
		public void SubscribeToMatchEvents()
		{
			...
			mMatchManager.OnNewMatch += OnMatchStart;
			mMatchManager.OnGol += OnGol;
            mMatchManager.OnFail += OnFail;
		...
		void OnGol (object sender, EventArgs e) {
			mAudioManager.PlayUniqueSoundDefinitionType(SoundDefinitions.CROWD_GOL);
		}
		...
		{	
			if ( e.State == InteractionStates.END ) {                
                if (e.ResultKind == InteractionResponseKind.GOOD)
                {
                    PlayVoicePerfect();
                }
                else if (e.ResultKind == InteractionResponseKind.BAD)
                {
                    PlayMissAction();
                }else if (e.ResultKind == InteractionResponseKind.TOO_LATE)
				{
					PlayMissAction();
				}
